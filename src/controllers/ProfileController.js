...
const updateProfile = handleAsync(async (req, res, next) => {
  const userId = req.user.id;
  const { fullname, phone, address, dateOfBirth, gender, detailedAddress } =
    req.body;
  ...
  if (detailedAddress) {
    ...
    if (detailedAddress.street)
      user.detailedAddress.street = detailedAddress.street;
    if (detailedAddress.ward) user.detailedAddress.ward = detailedAddress.ward;
    if (detailedAddress.district)
      user.detailedAddress.district = detailedAddress.district;
    if (detailedAddress.city) user.detailedAddress.city = detailedAddress.city;
    if (detailedAddress.country)
      user.detailedAddress.country = detailedAddress.country;
  }
...
const updatePassword = handleAsync(async (req, res, next) => {
  ...
  if (!currentPassword || !newPassword) {
    return next(
      createError(400, "Mật khẩu hiện tại và mật khẩu mới là bắt buộc")
    );
  }
  ...
  res.status(200).json({
    success: true,
    message:
      message.USER?.UPDATE_PASSWORD_SUCCESS || "Cập nhật mật khẩu thành công",
  });
});
...
const updateAvatar = handleAsync(async (req, res, next) => {
  ...
  res.status(200).json({
    ...
    data: updatedUser,
    message:
      message.USER?.UPDATE_AVATAR_SUCCESS || "Cập nhật avatar thành công",
  });
});
